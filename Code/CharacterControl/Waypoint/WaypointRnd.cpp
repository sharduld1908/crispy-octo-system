...
namespace CharacterControl{
	...
	namespace Events{
		...
		{
			static const struct luaL_Reg l_Event_CREATE_RND_WAYPOINT[] = {
				{"Construct", l_Construct},
				{NULL, NULL} // sentinel
			};

			// register the functions in current lua table which is the table for Event_CreateSoldierNPC
			luaL_register(luaVM, 0, l_Event_CREATE_RND_WAYPOINT);
		}

		int Event_CREATE_RND_WAYPOINT::l_Construct(lua_State* luaVM)
		{
			PE::Handle h("EVENT", sizeof(Event_CREATE_RND_WAYPOINT));
			Event_CREATE_RND_WAYPOINT *pEvt = new(h) Event_CREATE_RND_WAYPOINT;
	...
	namespace Components {
		...
		void RndWayPoint::addDefaultComponents()
		{
